...
func _run() -> void:
	...
	state.defense_level = 5
	state.buildings.wall = 5
	...
	for era_capture in [["qin", "qin"], ["han", "han"], ["three_kingdoms", "three_kingdoms"], ["jin", "jin"], ["northern_southern", "northern_southern"], ["sui", "sui"], ["tang", "tang"], ["five_dynasties", "five_dynasties"], ["song", "song"], ["yuan", "yuan"], ["ming", "ming"], ["qing", "qing"]]:
		state.chapter = 5
		state.era_progress = state.get_era_progress_target()
	...
	if failures.is_empty():
		print("VISUAL_CAPTURE_OK seasons=3 eras=14 intelligence=5 orders=2 policies=4 feedback=1 onboarding=2 ledger=1 modals=5 size=540x960")
		quit(0)
	else:
