...
func _run() -> void:
	...
	var era_scenarios := {
		"qin": [
			{"id": "obsolete", "force": {"militia": 40, "archer": 15, "chariot": 5}, "wall": 2, "barracks": 2, "warehouse": 1, "morale": 68.0},
			{"id": "transition", "force": {"militia": 66, "archer": 24, "chariot": 6}, "wall": 3, "barracks": 3, "warehouse": 2, "morale": 74.0},
			{"id": "prepared", "force": {"militia": 65, "archer": 25, "chariot": 10}, "wall": 4, "barracks": 4, "warehouse": 5, "morale": 80.0},
		],
		"han": [
			{"id": "obsolete", "force": {"militia": 48, "archer": 18, "chariot": 5}, "wall": 2, "barracks": 2, "warehouse": 1, "morale": 68.0},
			{"id": "transition", "force": {"militia": 64, "archer": 25, "chariot": 10}, "wall": 3, "barracks": 3, "warehouse": 1, "morale": 75.0},
			{"id": "prepared", "force": {"militia": 72, "archer": 30, "chariot": 15}, "wall": 4, "barracks": 4, "warehouse": 5, "morale": 82.0},
		],
	}
	for era_id in era_scenarios:
		...
		for scenario in era_scenarios[era_id]:
			...
			print("IMPERIAL_BATTLE_CASE %s/%s win=%.1f%% own=%.2f enemy=%.2f" % [era_id, scenario.id, metrics.win_rate * 100.0, metrics.player_losses, metrics.enemy_losses])
	...
	if failures.is_empty():
		print("IMPERIAL_BATTLE_BALANCE_OK runs=%d eras=%d" % [RUNS, era_scenarios.size()])
		quit(0)
	else:
