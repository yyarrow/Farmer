...
`tests/visual_capture.gd` 使用真实渲染器输出存档启动首页、十二建筑满城、春秋冬季、“水利、军队、伤营、近敌、政令动效”组合状态、十四时代城景与军务页，以及最长事件、战报等弹窗的 540×960 画面到 `.qa/`。`tests/visual_city_slots.gd` 另为十四时代分别输出空城、六建筑和十二建筑三种密度，共 42 张验收图，并输出道路禁建、有效迁建和 v6 八建筑修复布局图，用于逐时代检查地形空地、落脚线、透视方向、HUD 遮挡和边缘巡视。
