...
存档 v5 在 v4 的 `era_id`、`era_progress` 与 `city_level` 之上新增 `building_instances`。每座建筑拥有稳定实例 ID、类型、独立等级和槽位；同类生产建筑可重复营造，城垣保持唯一，`buildings` 聚合值只供经济和旧 API 兼容。校验器会拒绝实例 ID/槽位重复、占用未开放用地、唯一建筑重复或聚合等级不一致的存档；v4 旧建筑会按默认落点迁入开放槽位。
