...
- `src/data/eras/`：春秋、战国各自声明城池等级、建筑、兵种、敌军、阵令、资源单位、绘卷背景、事件和初始值。工厂方法每次返回独立可变数据。
...
时代与城池双成长已经落地。每个新朝代提供与 `spring_autumn.gd` 同结构的目录配置，并在 `era_registry.gd` 中登记顺序；时代切换器只改变当前目录，不把朝代判断散落到经济、战斗或 UI。当前三个内部兵种 ID 表示近战、远射、机动三个可跨时代继承的军籍角色，显示名称、数值、成本与敌军称谓均由时代配置决定。城池等级是同一时代内的空间成长，配置建筑槽位、繁荣目标、城景缩放与可见范围。
