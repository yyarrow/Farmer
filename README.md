...
- 春夏秋冬四首原创五声音阶国风音乐（共 192 秒），每首以 2 秒交叉衔接无缝循环，换季时 2.4 秒平滑切换；战斗音效会短暂压低背景音乐
- 总音量、背景音乐、操作音效、静音和触觉反馈设置，设置写入带损坏恢复且拖动时不反复刷盘
- 八类建筑六级可辨外观；建造、升级、买卖、征兵、政令、巡剿、日结、事件和战斗都有对应城景反馈，水利、军民、伤营与近敌状态会持续留在地图上
- 建筑图按手机实际显示尺寸保留三倍采样精度，降低低端设备常驻纹理与安装包负担
- 自动存档、三个手动存档槽及载入、覆盖、删除、重新开始确认；写入采用临时文件与上一版备份，截断或结构损坏时自动恢复
...
安卓包名为 `com.qinghe.farmer`，当前版本 `0.5.0`，最低 Android 7.0。旧版存档读取时会自动迁移为按人计数的新军籍和新资源制。存档和诊断只写入应用私有目录，不请求网络权限。诊断报告由玩家主动复制后发送，不会自动上传。
