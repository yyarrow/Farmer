...
- 主城随城池等级扩大并可左右拖动巡视；已实现的十四个时代均有独立绘卷背景，以及各自的城建、资源、军制、阵令、敌军与事件文案
...
安卓包名为 `com.qinghe.farmer`，当前版本 `0.12.0`，最低 Android 7.0。旧版存档读取时会依次迁移为按人计数的军籍/资源制和 v4 时代/城池双成长存档；既有进度默认归入春秋并按日期、城建、城池与战绩推导时代积累。没有阵令字段的旧档默认使用「持重」。存档和诊断只写入应用私有目录，不请求网络权限。诊断报告由玩家主动复制后发送，不会自动上传。
